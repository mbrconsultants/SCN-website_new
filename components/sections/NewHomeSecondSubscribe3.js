const NewHomeSecondSubscribe3 = () => {
    return (
        <>
            <section className="subscribe-section-two m-0 p-0" style={{background: "white"}}>
                <div className="large-container p-0">
                    <div className="outer-box wow fadeIn">
                        <div className="row"> 
                            <div className="form-column col-lg-6 col-md-12 order-lg-2 wow fadeInRight">
                                <div className="inner-column">
                                    <div className="sec-title light">
                                        <h3 className="text-white">To stay updated subscribe to our <br/>Newsletter</h3>
                                    </div>
                                    <div className="subscribe-form-two">
                                        <form method="post" action="#">
                                            <div className="form-group">
                                                <input type="email" name="emailaddress" placeholder="Subscribe Here" required/>
                                                <button type="submit" className="submit-btn overlay-anim"><i className="icon fa fa-paper-plane"></i></button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                            </div>
                            <div className="image-column col-lg-6 col-md-12 wow fadeInLeft">
                                <div className="inner-column">
                                    <figure className="image"><img src="images/resource/subscribeFooter2.png" style={{height:'350px'}} alt=""/></figure>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </>
    );
};
export default NewHomeSecondSubscribe3