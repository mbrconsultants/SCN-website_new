...
const BulletinList = () => {
  ...
  const getData = async () => {
    try {
      ...
      if (res.data && res.data.data.bulletins && Array.isArray(res.data.data.bulletins.data)) {
    ...
    <>
      <section className="about-section-nine">
        <div className="auto-container">
          <div className="row align-items-start"> {/* Ensure alignment at the top */}
              ...
              {data.length > 0 ? (
                data.map((bulletin, index) => (
                  ...
                  <div className="sec-title" style={{ paddingBottom: '10px' }}> {/* Reduce padding between the title and content */}
                    ...
                    <div>
                      <a href={bulletin.file_path} target="_blank" rel="noopener noreferrer">
                        <figure className="image">
                          <img src="images/resource/icon.jpg" alt="PDF Icon" style={{ height: '50px', width: '50px' }} />
                        </figure>
                      </a>
                    </div>
                  </div>
